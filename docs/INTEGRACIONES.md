...
- **Poll (backstop):** `GET /api/cron/toast-sales` (`Authorization: Bearer $CRON_SECRET`) polls `ordersBulk` by modified time since a watermark, 10-min overlap. **First run sets the watermark to _now_ and posts nothing — no historical backfill.** `?dryRun=1&hours=N` previews without writing. **Cron cadence:** daily (`20 4 * * *`) because Vercel Hobby only allows daily crons — a sub-daily backstop (e.g. every 15 min) needs Vercel Pro. The webhook is the real-time path regardless; the poll just catches anything missed. Can also be triggered manually anytime with the bearer.
